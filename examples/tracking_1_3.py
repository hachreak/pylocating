...
log_config = "examples/tracking_1_3.json"
...
env1 = Environment(config=env1_config, base=base, radius=radius)
env2 = Environment(config=env2_config, base=base, radius=radius)
env1.registerNeighbor(env2)
# particle position generator
particle_position_generator = around_beacons(env1)
...
# engine for env2
engine_2 = ParticleEngine(
    config={},
    environment=env2,
    stop_condition=(lambda engine: False)
)

# object moving simulator
mpe = LoggingMovingPointEngine(
    config={
        'sleep': 3
    },
    listener=EnvironmentListener(environment=env1, error=1),
    start_point=point,
